...
This library, designed to help working with root for analysis, contains several GUI analysis tools and useful sets of functions. An explanation of some of the major components is given here, but the subclasses and functions will not be detailed.
Within the GUI classes, hover over controls to view explanatory tooltip.
...
The gating tool is designed to provide live graphical gating and background subtraction for \textit{TH2}, \textit{TH3} and  \textit{THn} histograms. 
A new instance can be created from the jEnv toolbar or by typing any of the following functions in the terminal:
\lstset{language=C++}
\begin{lstlisting}
jGatingToolSelector();
jGatingToolSelector(TObject*);
jGatingToolSelector("HistogramName");
\end{lstlisting}
where "HistogramName" is the name (not title) of a histogram open in memory. TOject* should be a pointer to an object of type inheriting from TH2, TH3 or THnBase (e.g. THnSparse). The call without arguments will grab the most recently selected or drawn histogram. If no valid input is found, a window will not appear. 
\begin{center}
...
For the first gate on a \textit{TH3} there is significant amount of computation so, by default the gate window will not execute until the [Update] button is pressed, or when the background mode is changed. If your PC can handle live updating during gate adjustment, the setting can be overridden with the global function \textit{SetGlobalTH3UpdateLock(false)}, before starting the gating tool instance.
As the 3D tool can be too wide for some displays, the system configuration can be set such that the first gate appears as a separated window, by use of the global function \textit{SetGlobalTH3UseHead()}. 
\begin{center}
...
\subsection{Underflow, Overflow}
Unlike previous versions, the decision of whether or not to include the Overlow and Underflow bins, which are excluded by default, can be set with global functions;
\textit{SetGlobalOverflowInclude(bool)} and \textit{SetGlobalUnderflowInclude(bool)}.
This will affect the initial projection and hence change the background subtraction fraction. 
When setting your gates, inclusion of the Over/Underflow bins of the relevant axis can be set, regardless of the global parameters, by using the manual background mode. For ``Compton'' background overflow is always included. ``Full'' and ``Anti'' background modes will use the global settings. 
For spectroscopic histograms (energy axes), it is recommended to include Overflow bins.

\subsection{Background Fraction Error}
A key feature of the gating tool is the correct propagation of statistical errors.
This includes an error associated with the uncertainty in the background fraction. A global ``best guess'' value of 4\% is used.
The value can be manually set, or turned off, with the global function \textit{SetGlobalSubtractionFractionError(0.04)}.
The error is doubled when the manual background fraction, rather than a fit, is used.
